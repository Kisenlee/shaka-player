...
shaka.cea.Cea708Service = class {
  ...
  handleC1_(dtvccPacket, captionCommand, pts) {
    // Note: This decoder processes data immediately, so the delay (0x8d) and
    // delayCancel (0x8e) commands do not affect timing. However, the delay
    // command carries a 1-byte operand that MUST be consumed so the
    // service-block byte counter stays aligned for subsequent commands.
  ...
  /**
   * Handles the C1 Delay (DLY, 0x8d) command. The command is followed by a
   * single operand byte specifying the delay in tenths of a second. This
   * decoder processes data immediately and applies no delay, but the operand
   * byte MUST still be read so the service-block byte counter remains aligned
   ...
   */
  delay_(dtvccPacket) {
    // Consume (and discard) the single delay operand byte.
    dtvccPacket.readByte();
  }

  /**
   * Handles the C1 DelayCancel (DLC, 0x8e) command. This command has no operand
   * byte. Since this decoder applies no delays, this is a no-op.
   * @private
   */
  delayCancel_() {
    // No operand byte to consume, and no delay state to cancel.
  }
  ...
  setPenAttributes_(dtvccPacket) {
    ...
    // Map pen size, font tag, and edge type to the nearest representable cue
    // style on the window's pen state (CTA-708-E §8.5.3). These are stored on
    // the window so cue emission can use them where representable.
  ...
  setWindowAttributes_(dtvccPacket) {
    ...
    // A transparent fill (opacity id 3) means no window background is drawn, so
    // clear any fill color. Otherwise map to the nearest representable color.
    const fillColor =
        fillOpacity === shaka.cea.Cea708Service.FILL_OPACITY_TRANSPARENT ?
        '' : this.rgbColorToHex_(fillRed, fillGreen, fillBlue);
    this.currentWindow_.setWindowFillColor(fillColor);

    // Extract and store the word-wrap flag (bit 0x40 of byte 3). Full word-wrap
    // layout behavior is not modeled, but the bit is read and stored so the
    // window state matches the stream (CTA-708-E §8.4.2).
  ...
  defineWindow_(dtvccPacket, windowNum, pts) {
    ...
    // Apply the predefined window style (WNSTY). Per CTA-708-E, a style id of 0
    // keeps the previous style of an existing window; for a newly created
    // window it defaults to style 1. Passing 0 for an existing window is a
    // no-op inside applyWindowStylePreset_, which keeps the result a
    // deterministic function of the preset id (and prior style only when the
    // id is 0 and the window already existed).
    this.applyWindowStylePreset_(window,
        windowAlreadyExists ? windowStyle : (windowStyle || 1));

    // Apply the predefined pen style (PNSTY). A pen style of 0 on an existing
    // window keeps its current pen; otherwise style (penStyle || 1) is applied.
  ...
  /**
   * Applies a predefined window style (WNSTY) to a window, as defined by
   * CTA-708-E §8.6.2. The result is a deterministic function of the preset id:
   * a preset id in [1,7] applies the corresponding entry from
   * {@link shaka.cea.Cea708Service.WindowStylePresets}; a preset id of 0 leaves
   * the window's current style unchanged (the caller resolves the "new window"
   * case to preset 1 before calling).
   *
   * Note: this decoder derives a cue's text justification from the
   * SetWindowAttributes (0x97) command and renders text left-to-right, so the
   * print direction, scroll direction, and word-wrap of a predefined style are
   * stored on the window for region handling but do not change the cue's
   * justification here. This preserves the decoder's established default
   * rendering for windows defined without an explicit style.
  ...
  applyWindowStylePreset_(window, presetId) {
    if (presetId === 0) {
      // Keep the window's existing style.
      return;
    }
  ...
  /**
   * Applies a predefined pen style (PNSTY) to a window, as defined by
   * CTA-708-E §8.6.3. The result is a deterministic function of the preset id:
   * a preset id in [1,7] applies the corresponding entry from
   * {@link shaka.cea.Cea708Service.PenStylePresets}; any other value falls back
   * to preset 1 (the default pen).
   *
   * The pen color, italics, and underline are reset to their defaults (matching
   * the decoder's prior behavior of resetting the pen for a predefined style),
   * and the preset's pen size, font style, and edge type are stored on the
   * window for cue styling.
   * @param {!shaka.cea.Cea708Window} window
  ...
  applyPenStylePreset_(window, presetId) {
    ...
    // Reset color/italics/underline (and pen location) to defaults, preserving
    // the previous behavior of resetting the pen when a style is applied.
    window.resetPen();
