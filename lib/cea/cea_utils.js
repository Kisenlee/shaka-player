...
shaka.cea.CeaUtils = class {
   ...
   *   to a static bold style (no blinking, which is out of scope). Optional;
  ...
  static createStyledCue(startTime, endTime, underline,
    ...
    if (flash) {
      // CEA-608 Flash-On has no animated representation in the cue model, so
      // it is surfaced as a static emphasis (bold). CEA-608 never otherwise
      // sets font weight, so non-flashing cues keep the default weight.
...
shaka.cea.CeaUtils.StyledChar = class {
   ...
   *   Flash-On (FON) run. Optional; defaults to false so existing callers
   *   keep their current (non-flashing) behavior.
   */
  constructor(character, underline, italics, backgroundColor, textColor,
    ...
    /**
     * Whether this character belongs to a CEA-608 Flash-On (FON) run. The
     * decoder has no concept of animated blinking (out of scope), so this is
     * carried through to a static cue style downstream.
     * @private {boolean}
     */
    this.flash_ = flash;
  }
