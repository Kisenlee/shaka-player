...
shaka.cea.Cea608DataChannel = class {
  ...
  controlPac_(b1, b2) {
    ...
    // A PAC establishes a new base position for the row, so any tab offset from
    // a previous line is cleared here. A following tab-offset (TO1–TO3) control
    // code, which the spec places after the PAC, sets the offset again. This
    // keeps a stale offset from leaking into a later caption that has none.
    buf.setOffset(null);
    ...
    // A PAC resets the base style, which ends any active Flash-On (FON) run.
    buf.setFlash(false);
  }
   ...
   */
  controlFon_(time) {
    // Flash-On begins a flashing run: mark subsequently added characters with
    // the flash style attribute. No animated blinking is rendered (out of
    // scope); downstream this maps to a static cue style. Like other mid-row
    // codes, FON also advances the cursor by inserting a space.
   ...
   * Erases characters from the cursor to the end of the current row in the
   * active buffer, leaving all other cells and the cursor position unchanged.
   * @private
   */
  controlDer_() {
